    ...
    '<div class="doc-card" data-brand="' + escapeHTML(doc.brand) + '" data-cat="' + type + '">' +
      ...
      '<div class="doc-footer">' +
        ...
        '<a href="' + escapeHTML(doc.url) + '" ' +
          ...
          ' data-category="' + escapeHTML(doc.category || type) + '"' +
          ' data-brand="' + escapeHTML(doc.brand || '') + '"' +
          ' data-title="' + escapeHTML(doc.title || '') + '"' +
          ' data-cert-type="' + escapeHTML(doc.certType || '') + '">' +
          btnIcon + ' ' + btnLabel +
        '</a>' +
      '</div>' +
    '</div>';
...
window.openBrand = function(catKey, brand) {
  ...
  // Tracking analytics
  if (typeof window.trackBrandOpen === 'function') {
    window.trackBrandOpen(catKey, brand);
  }
};
...
window.handleOpen = function(e, btn) {
  ...
  // Tracking analytics — só executa se trackDocOpen estiver disponível
  if (typeof window.trackDocOpen === 'function') {
    window.trackDocOpen({
...
function showTab(tabName) {
  ...
  // Tracking analytics
  if (typeof window.trackTabChange === 'function') {
    window.trackTabChange(tabName);
  }
}
...
function setupSearch(inputId, catKey) {
  ...
  input.addEventListener('input', () => {
    renderCategoryView(catKey);
    // Tracking (com debounce próprio em analytics.js)
    if (typeof window.trackSearch === 'function') {
      window.trackSearch(catKey, input.value);
    }
  });
}
...
function setupSuggestForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    // Tracking analytics
    if (typeof window.trackSuggestion === 'function') {
      window.trackSuggestion(category);
    }
  });
}
